...
public final class Admin {
    ...
    public void setSongs(final List<SongInput> songInputList) {
        ...
        for (SongInput songInput : songInputList) {
            songs.add(new Song.Builder(songInput.getName(), songInput.getDuration())
    ...
    public void setPodcasts(final List<PodcastInput> podcastInputList) {
        ...
        for (PodcastInput podcastInput : podcastInputList) {
            ...
            Podcast podcast = new Podcast(podcastInput.getName(),
    ...
    /**
     * Adds a specified Merch object to the collection.
     *
     * @param merches the Merch object to be added. Cannot be null.
     */
    public void setMerches(final Merch merches) {
        this.merches.add(merches);
    }


    /**
     * Retrieves a Merch object by its name.
     * Returns null if no matching Merch is found.
     * Equality is determined by the equals() method of Merch.
     *
     * @param merchName the name of the Merch to retrieve.
     * @return the found Merch object, or null if not found.
     */
    public Merch getMerch(final String merchName) {
        Merch merch = new Merch(merchName, "", 0);
        int index = merches.indexOf(merch);
        if (index == -1) {
            return null;
        } else {
            return merches.get(index);
        }
    }
